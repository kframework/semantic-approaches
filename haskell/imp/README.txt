...
In each folder there are certain files:
	...
	- Imp_Bigstep.hs 	defines bigstep semantics of the language, allowing programs to be run.
...
Thoughts on implementing big step semantics in Haskell:
...
I think smallstep semantics would probably be the same amount of work for the same benefit as bigstep in Haskell, but denotational semantics would be very tricky to do. It's already so verbose in Maude - it would become even more verbose in Haskell.
