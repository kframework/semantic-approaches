...
module Parser (parseString, parseFile) where
    ...
    -- NEW This converts an int statement into a series of Let statements
    intStmt :: Parser Stmt
    intStmt =
        do {
            reserved "int" ;
            ...
            let build list = case List.uncons list of
                                Nothing -> stmt
                                Just (x, xs) -> Let x (AConst 0) (build xs) in
            return $ build vars
        }
    ...
    program :: Parser Pgm
    program =  -- This parsing is kept so that the final state is viewable at the end. The first 3 lines of the do and storing the vars in Init are technically not needed anymore, but then
               -- the first let would go out of scope and the end result would be an empty state.
        do  {
                reserved "int" ;
                vars <- commaSep identifier ;
                semi ;
