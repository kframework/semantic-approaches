...
module Parser (parseString, parseFile) where
    ...
    aOperators =    [   [Prefix (reservedOp "++"  >> return (PlusPlus         ))          ], -- NEW. This has to return an AExp which is why that's used in the syntax.
    ...
    program :: Parser Pgm
    program = 
        do  { -- Kept in although unneeded b/c locals parsing, see Parser.hs in part 5
                reserved "int" ;
                vars <- commaSep identifier ;
                semi ;
                stmt <- statement ;
