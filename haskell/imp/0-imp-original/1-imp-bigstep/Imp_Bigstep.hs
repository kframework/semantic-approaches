...
module Imp_Bigstep (execute, Configuration (ConfPgm)) where
    ...
    -- A state is a mapping from strings (identifiers) to integers (values)
    ...
    -- There's no special non determinism handling because it wouldn't matter: the language has no side effects. One can see that these semantics are much simpler
    -- than the semantics needed to handle determinism in part 1, 2, 4, and 6 since there's no need to map the cases on lists.
